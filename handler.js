...
this.reply(m.chat, `تم بيع الماس الخاص بك اليوم ⚠️\n\nيرجى شراء الكتابة الماس ${usedPrefix}buy o ${usedPrefix}buyall`, m)
continue
}
...
if (m.diamond)
m.reply(`💎 وقد تم استخدام أ *${+m.diamond}* الماس`)
}
break
}
}
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@date', global.fecha).replace('@time', global.tiempo).replace('@readMore', global.readMore).replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*⚠️ هذه المجموعة ليس لها وصف ⚠️*') :
...
"thumbnail": apii.data, 
"title": `${action === 'add' ? '乂 مرحبا بك 乂' : '乂 ويلقم 乂'}`,
"body": packname,
...
case 'promote':
case 'promover':
text = (chat.sPromote || this.spromote || conn.spromote || '@user الآن هو المسؤول')
case 'demote':
case 'degradar':
if (!text)
text = (chat.sDemote || this.sdemote || conn.sdemote || '@user لم يعد مسؤولاً')
...
if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || conn.sDesc || 'وصف التغيير \n@desc').replace('@desc', groupUpdate.desc)
if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || 'اسم المجموعة يتغير  \n@group').replace('@subject', groupUpdate.subject)
if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || 'أيقونة المجموعة تتغير ').replace('@icon', groupUpdate.icon)
if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'تغييرات رابط المجموعة \n@revoke').replace('@revoke', groupUpdate.revoke)
if (!text) continue
...
let msg = {
rowner: '*¡الامر دا ببمطور بس يحبب🫱🏽‍🫲🏻!*',
owner: '*¡مطور بس!*',
mods: '*¡يمكن للمشرفين فقط استخدام هذه القيادة!*',
premium: '*¡يمكن للمستخدمين المتميزين فقط استخدام هذه الوظيفة!*',
group: '*¡هذا الأمر منفردًا ويمكن استخدامه في المجموعة!*',
private: '*¡هذه الوظيفة يمكن استخدامها فقط في الدردشة الخاصة!*',
admin: '*¡خد ادمن طه الاول + دز يعضو🗿!*',
botAdmin: '*¡ات ادمن ياه!*',
unreg: '> *¡تسجيل جديد!*\n\n#verificar الاسم.العمر\n\n*الاستخدام الصحيح:* .تسجيل شاورما.20',
